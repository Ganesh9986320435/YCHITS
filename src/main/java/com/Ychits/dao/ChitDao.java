...
public class ChitDao {
    ...
    public ResponseEntity<ApiResponse> getChitsAssociatedTo(String userId) {
        return ResponseUtil.getOkResponse(chitRepository.findAll().stream().filter(chit -> Objects.nonNull(chit.getUserId())?chit.getUserId().contains(userId):false).toList());
    }
